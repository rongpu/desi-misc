...
plot_dir = '/global/cfs/cdirs/cosmo/www/temp/rongpu/ibis/ibis_dr1_subsets/deep_fields_full_depth'
...
for band in ibis_filters:
    ...
    fig_efftime, axes_efftime = plt.subplots(1, 2, figsize=(15, 6))
    fig_efftimeh, axes_efftimeh = plt.subplots(1, 2, figsize=(11, 5))
    fig_nexp, axes_nexp = plt.subplots(1, 2, figsize=(15, 6))
    fig_nexph, axes_nexph = plt.subplots(1, 2, figsize=(11, 5))
    fig_depth, axes_depth = plt.subplots(1, 2, figsize=(15, 6))
    fig_depthh, axes_depthh = plt.subplots(1, 2, figsize=(11, 5))
    fig_seeing, axes_seeing = plt.subplots(1, 2, figsize=(15, 6))
    fig_seeingh, axes_seeingh = plt.subplots(1, 2, figsize=(11, 5))

    for field_index in range(len(field_names)):
        field = field_names[field_index]
        print(field)

        nexp_grid, efftime_grid, depth_grid, seeing_grid, ra, dec = get_depth_map(band, field)

        maps = Table()
        maps['ra'], maps['dec'] = ra, dec
        maps['nexp'], maps['efftime'], maps['depth'], maps['seeing'] = nexp_grid, efftime_grid, depth_grid, seeing_grid
        maps.write('/global/cfs/cdirs/desicollab/users/rongpu/data/ibis/deep_field_subsets/approximate_depth_maps/full_depth_{}_{}.fits'.format(field.lower(), band), overwrite=True)
        ...
        # Get median values with 1.6 deg radius
        search_radius = 1.6*3600.
        ra_center, dec_center = field_centers[field]
        idx1, idx2, d2d, _, _ = search_around([ra_center], [dec_center], ra, dec, search_radius=search_radius, verbose=False)
        mask_central = np.full(len(ra), False)
        mask_central[idx2] = True
        ...
        xx = efftime_grid
        ax.hist([xx[mask_central & mask], xx[mask]], 100, label=['within 1.6 deg radius', 'all'], histtype='stepfilled', color=['C1', 'C0'])
        ax.set_xlabel('{}-band effective time'.format(band))
        ax.set_title('{} median={:.2f}; nmad={:.2f}'.format(field, np.median(xx[mask_central & mask]), nmad(xx[mask_central & mask])))
        ...
        ax.hist([xx[mask_central & mask], xx[mask]], bins=bins, label=['within 1.6 deg radius', 'all'], histtype='stepfilled', color=['C1', 'C0'])
        ax.set_xlabel('{}-band nexp'.format(band))
        ax.set_title('{} median={}; nmad={:.1f}'.format(field, np.median(xx[mask_central & mask]), nmad(xx[mask_central & mask])))
        ax.axvline(np.median(xx[mask_central & mask]), ls='--', lw=1, color='C3')
        if field_index==0:
            ax.legend(loc='best')
        ax.grid(alpha=0.5)
        ...
        mask = np.isfinite(depth_grid)
        xx = depth_grid
        ax.hist([xx[mask_central & mask], xx[mask]], 100, label=['within 1.6 deg radius', 'all'], histtype='stepfilled', color=['C1', 'C0'], range=(23.5, 26))
