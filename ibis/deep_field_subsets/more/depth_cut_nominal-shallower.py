# 0.2 mag deeper/shallower than the nominal 25.0 mag depth
# 
...
# shallower by 0.2 mag
delta_depth = -0.2
for key in nominal_efftime_dict.keys():
    nominal_efftime_dict[key] *= 10**(0.8*delta_depth)
...
grid.write(f'/global/cfs/cdirs/desicollab/users/rongpu/data/ibis/deep_field_subsets/misc/depth_and_efftime_subset_24.8_{field}_{band}.fits', overwrite=True)

ccd = ccd[ccds_to_keep]
ccd.write(f'/global/cfs/cdirs/desicollab/users/rongpu/data/ibis/deep_field_subsets/misc/survey-ccds-ibis-dr1-subset_24.8_{field}_{band}.fits', overwrite=True)
