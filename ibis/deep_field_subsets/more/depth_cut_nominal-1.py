# Exclude the CCDs from depth_cut_nominal.py, thus allowing for independent measurements
...
# exclude CCDs from the original run
print(len(ccd))
ccd_exclude = Table(fitsio.read(f'/global/cfs/cdirs/desicollab/users/rongpu/data/ibis/deep_field_subsets/misc/survey-ccds-ibis-dr1-subset_25.0_{field}_{band}.fits'))
ccd['ccd_id_str'] = np.char.add(np.array(ccd['expnum']).astype(str), ccd['ccdname'])
ccd_exclude['ccd_id_str'] = np.char.add(np.array(ccd_exclude['expnum']).astype(str), ccd_exclude['ccdname'])
mask = ~np.in1d(ccd['ccd_id_str'], ccd_exclude['ccd_id_str'])
ccd = ccd[mask]
print('Exclude CCDs from the original run', len(ccd))
...
grid.write(f'/global/cfs/cdirs/desicollab/users/rongpu/data/ibis/deep_field_subsets/misc/depth_and_efftime_subset_25.0_{field}_{band}-1.fits', overwrite=False)

ccd = ccd[ccds_to_keep]
ccd.write(f'/global/cfs/cdirs/desicollab/users/rongpu/data/ibis/deep_field_subsets/misc/survey-ccds-ibis-dr1-subset_25.0_{field}_{band}-1.fits', overwrite=False)
