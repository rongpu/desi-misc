...
field_ra, field_dec, field_rad_deg = 35.75, -4.75, 1.7
area = np.pi * field_rad_deg ** 2

fnodet = 0.001
magnodet = 22.5 - 2.5 * np.log10(fnodet) # 30

fn = '/dvs_ro/cfs/cdirs/desi/users/raichoor/ibis/ibis3-xmm-forced-hsc-wide.fits'
...
fibmagmin, fibmagmax, fibmagmax_bright = 22.0, 25.25, 25.0
mb_colmin = 0.1
ri_colmax = 0.35
gr_colmax = 0.7
...
# brighter IBIS limit
mask &= (cat['M411fibmag'] > fibmagmin) & (cat['M411fibmag'] < fibmagmax_bright)
print("{:.1f}k/deg2\tafter {} < fibmag_bright < {} cut".format(mask.sum() / 1000. / area, fibmagmin, fibmagmax))
cat['M411_sel_bright'] = mask.copy()
...
# brighter IBIS limit
mask &= (cat['M438fibmag'] > fibmagmin) & (cat['M438fibmag'] < fibmagmax_bright)
print("{:.1f}k/deg2\tafter {} < fibmag_bright < {} cut".format(mask.sum() / 1000. / area, fibmagmin, fibmagmax))
cat['M438_sel_bright'] = mask.copy()
...
# brighter IBIS limit
mask &= (cat['M464fibmag'] > fibmagmin) & (cat['M464fibmag'] < fibmagmax_bright)
print("{:.1f}k/deg2\tafter {} < fibmag_bright < {} cut".format(mask.sum() / 1000. / area, fibmagmin, fibmagmax))
cat['M464_sel_bright'] = mask.copy()
...
# brighter IBIS limit
mask &= (cat['M490fibmag'] > fibmagmin) & (cat['M490fibmag'] < fibmagmax_bright)
print("{:.1f}k/deg2\tafter {} < fibmag_bright < {} cut".format(mask.sum() / 1000. / area, fibmagmin, fibmagmax))
cat['M490_sel_bright'] = mask.copy()
...
# brighter IBIS limit
mask &= (cat['M517fibmag'] > fibmagmin) & (cat['M517fibmag'] < fibmagmax_bright)
print("{:.1f}k/deg2\tafter {} < fibmag_bright < {} cut".format(mask.sum() / 1000. / area, fibmagmin, fibmagmax))
cat['M517_sel_bright'] = mask.copy()
...
assert np.all(cat['RA']==tmp['RA']) and np.all(cat['DEC']==tmp['DEC']) and np.all(cat['lae_sel']==tmp['lae_sel'])

cols = list(np.array(cat.colnames)[~np.in1d(cat.colnames, tmp.colnames)])
cols = ['RA', 'DEC']+cols
cat = cat[cols]

cat.write('/global/cfs/cdirs/desicollab/users/rongpu/data/desi2/xmm_highz/tertiary49_xmm_lae_targets-more.fits', overwrite=False)
