...
assert np.all((cat['lae_sel_bright'] & cat['lae_sel'])==cat['lae_sel_bright'])

mask = cat['lae_sel'].copy()
cat = cat[mask]
print(len(cat))

cat.write('/global/cfs/cdirs/desicollab/users/rongpu/data/desi2/tertiaryX/tertiaryX_cosmos_lae_filler_targets.fits', overwrite=False)
