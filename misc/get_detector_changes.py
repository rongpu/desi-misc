#!/usr/bin/env python3
"""
Script to extract detector changes from DESI spectrograph YAML files.
Returns a dictionary of detector changes with start and end dates.
"""
...
def get_detector_changes(base_path=None, output_path=None):
    """
    Parse YAML files and extract detector changes.
    ...
    output_path : str, optional
        If provided, save the detector changes dictionary to this path in YAML format.
    ...
    for yaml_file in yaml_files:
        ...
        # Extract detector and date information for each version
        detector_entries = []
        ...
        for i, entry in enumerate(detector_entries):
            ...
            # Add entry if detector changed or it's the first entry
        ...
        for change in changes:
            # Get detector name (the only key in the dict)
        ...
        if merged_changes:
            detector_changes[camera_name] = merged_changes

    # Save to file if output_path is provided
    if output_path:
        with open(output_path, 'w') as f:
            yaml.dump(detector_changes, f, default_flow_style=False, sort_keys=False)

    return detector_changes
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        ...
        epilog='''
Examples:
  # Print detector changes to screen
  python get_detector_changes.py

  # Save to YAML file
  python get_detector_changes.py -o detector_changes.yaml

  # Use custom path
  python get_detector_changes.py -p /path/to/spec -o output.yaml

Environment variables:
  DESI_SPECTRO_CALIB    Base calibration directory (default path: $DESI_SPECTRO_CALIB/spec)
        '''
    )
    ...
    # Get detector changes
    changes = get_detector_changes(base_path=args.base_path, output_path=args.output_path)

    # Print results (unless saving to file only)
    if not args.output_path:
        print("Detector Changes in DESI Spectrograph")
        print("=" * 50)
        print_detector_changes(changes)
    else:
        print(f"Detector changes saved to {args.output_path}")
