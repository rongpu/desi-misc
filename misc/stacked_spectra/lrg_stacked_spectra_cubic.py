    ...
    for petal in petals:
        ...
        for camera in ['B', 'R', 'Z']:
            ...
            for i, isp in zip(ii, iisp):
                interp_flux = interp1d(wsp / (1 + zs[i]), flsp[isp], kind='cubic', bounds_error=False, fill_value=0.)
                interp_ivar = interp1d(wsp / (1 + zs[i]), ivsp[isp], kind='cubic', bounds_error=False, fill_value=0.)
