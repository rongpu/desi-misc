    ...
    for petal in petals:
        ...
        for camera in ['B', 'R', 'Z']:
            ...
            for i, isp in zip(ii, iisp):
                interp_flux = interp1d(wsp / (1 + zs[i]), flsp[isp], kind='nearest', bounds_error=False, fill_value=0.)
                interp_ivar = interp1d(wsp / (1 + zs[i]), ivsp[isp], kind='nearest', bounds_error=False, fill_value=0.)
                rfflsp = interp_flux(rfws)
                rfivsp = interp_ivar(rfws)
                ws = rfivsp
                fl += rfflsp * ws
                iv += ws
                n[ws > 0] += 1
...
h.writeto('/global/cfs/cdirs/desi/users/rongpu/tmp/lrg_stacked_spectra_interp_nearest.fits', overwrite=True)
