...
plot_dir = '/global/cfs/cdirs/desi/users/rongpu/plots/dr9dev/pupil_pattern/lg_regular_cp/'
...
def make_plots(image_path_lg):
    ...
    image_filename = os.path.basename(image_path_lg)

    mask = ccd['image_filename_lg'] == image_filename
    image_filename_cp = ccd['image_filename'][mask][0]
    image_path_cp = os.path.join(image_dir, image_filename_cp)

    band = image_filename[image_filename.find('_ooi_')+5]
    vrange = image_vrange[band]

    plot_path = os.path.join(plot_dir, os.path.basename(image_filename_cp.replace('.fits.fz', '.png')))
