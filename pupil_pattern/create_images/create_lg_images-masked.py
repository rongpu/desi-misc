...
plot_dir = '/global/cfs/cdirs/desi/users/rongpu/plots/dr9dev/pupil_pattern/lg_masked/'
...
# image_path_list = glob.glob(os.path.join(lg_image_dir, '*ooi*.fits.fz'))
...
def make_plots(image_fn_lg):
    ...
    # image_filename = os.path.basename(lg_image_path)
    ...
    plot_path = os.path.join(plot_dir, image_fn_lg.replace('.fits.fz', '.png'))
    ...
    for ccdnum in ccdnum_list_for_median_sky:
        ...
        img = fits.getdata(lg_image_path, extname=ccdname)
    ...
    with fitsio.FITS(lg_image_path) as tmp:
...
def main():
    ...
    with Pool(processes=n_processes) as pool:
        res = pool.map(make_plots, image_fn_lg_list)

    # make_plots(image_fn_lg_list[0])
    # make_plots('c4d_170722_103109_ooi_z_lg9.fits.fz')
