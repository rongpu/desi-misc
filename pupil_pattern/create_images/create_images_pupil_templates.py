...
plot_dir = '/global/cfs/cdirs/desi/users/rongpu/plots/dr9dev/pupil_pattern/pupil_templates/'
...
def make_plots(run):
    ...
    mjd_span = ccd['mjd_obs'][mask].max() - ccd['mjd_obs'][mask].min()

    if raw:
        img_path = os.path.join(template_dir, 'pupil_raw_{}_{}.fits.fz'.format(band, run))
        plot_path = os.path.join(plot_dir, '{}_{}_pupil_template_raw.png'.format(band, run))
    else:
        img_path = os.path.join(template_dir, 'pupil_smooth_{}_{}.fits.fz'.format(band, run))
        plot_path = os.path.join(plot_dir, '{}_{}_pupil_template_smooth.png'.format(band, run))        
    ...
    for ii, ccdnum in enumerate(ccdnum_list):
        ...
        try:
            img = fits.getdata(img_path, extname=ccdname)
        except:
        ...
        if np.sum(~np.isfinite(img))>0:
            print('{} bad pixels'.format(~np.isfinite(img)))
        ...
        fig = plt.imshow(img.T, cmap='seismic', vmin=-0.1, vmax=0.1, 
