# Images from regular CP processing
...
# plot_dir = '/global/cfs/cdirs/desi/users/rongpu/plots/dr9dev/pupil_pattern/cp_pupil/'
plot_dir = '/global/cfs/cdirs/desi/users/rongpu/plots/dr9dev/pupil_pattern/cp_pupilmax/'
...
ccd_dr8 = Table.read('/global/cscratch1/sd/rongpu/dr9dev/pupil_pattern/survey-ccds-decam-dr8-pupil-params.fits')
mask = np.isfinite(ccd_dr8['PUPILAMP'])
print(np.sum(~mask))
...
for band in ['g', 'r', 'z']:
    ...
    for pupilmax in values:
        idx = np.argsort(np.abs(ccd['PUPILMAX'][mask_band]-pupilmax))
        expnum_list += list(ccd['expnum'][mask_band][idx[:n_plots]])
...
def make_plots(expnum):
    ...
    mask_band = ccd['filter']==band
    frac = np.sum(ccd['PUPILMAX'][mask_band]>ccd['PUPILMAX'][ccd_index])/np.sum(mask_band)

    text = 'expnum: {}\n'.format(ccd['expnum'][ccd_index])
    text += '# good CCDs: {}\n'.format(ccd['good_ccds'][ccd_index])
    text += 'PUPILMAX  PUPILAMP:\n'
    text += '{}  {}\n'.format(ccd['PUPILMAX'][ccd_index], ccd['PUPILAMP'][ccd_index])
    text += 'Fraction with larger PUPILMAX: {:.2f}%\n'.format(frac*100.)
    if expnum in ccd_dr8['expnum']:
        ccd_index_dr8 = np.where(ccd_dr8['expnum']==expnum)[0][0]
        text += 'DR8 PUPILMAX PUPILAMP:\n'
        text += '{} {}\n'.format(ccd_dr8['PUPILMAX'][ccd_index_dr8], ccd_dr8['PUPILAMP'][ccd_index_dr8])
    else:
        text += '\n\n'
    text += '\n\n'
    plt.text(1.08, 0.660, text, fontsize=15)
