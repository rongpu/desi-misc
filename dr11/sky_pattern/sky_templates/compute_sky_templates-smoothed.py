...
runs_without_n15 = []  # hot spot

raw_templates_dir = '/dvs_ro/cfs/cdirs/desi/users/rongpu/data/dr11dev/sky_templates_raw'
templates_dir = '/global/cfs/cdirs/desi/users/rongpu/data/dr11dev/sky_templates'
...
def compute_smooth_sky(run):
    ...
    raw_path = os.path.join(raw_templates_dir, 'sky_raw_{}_{}.fits'.format(band, run))
    ...
    for ccdnum in ccdnum_list:
        ...
        try:
            ...
            header = fitsio.read_header(raw_path, ext=ccdname)
        except:
            continue
        ...
        hdul_template.write(data=img_smooth_orig, extname=ccdname, compress='rice', header=header)
