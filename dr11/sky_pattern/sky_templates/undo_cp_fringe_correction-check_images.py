...
def decam_plot(image_path, plot_path, figsize=(26, 24), vrange=None, dr8=False, binsize=10, median=True,
    ...
    for ii, ccdnum in enumerate(ccdnum_list):
        ...
        fig = plt.imshow(img.T, cmap='gray', vmin=-vrange*1.5, vmax=vrange*1.5, 
...
fringe_list_path = '/global/cfs/cdirs/desi/users/rongpu/data/dr11dev/misc/fringe_list.txt'
...
new_image_dir = '/global/cfs/cdirs/desi/users/rongpu/data/dr11dev/undo_cp_fringe_corr'
plot_dir = '/global/cfs/cdirs/cosmo/www/temp/rongpu/dr11/undo_cp_fringe_corr'
...
def wrapper(fn):
    ...
    plot_path = os.path.join(plot_dir, os.path.basename(fn)).replace('.fits.fz', '.png')
    if not os.path.isfile(plot_path):
        decam_plot(image_path, plot_path)
    
    image_path = os.path.join(new_image_dir, fn)
    plot_path = os.path.join(plot_dir, os.path.basename(fn)).replace('.fits.fz', '_new.png')
    if not os.path.isfile(plot_path):
        decam_plot(image_path, plot_path)
