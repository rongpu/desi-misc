...
template_dir = '/dvs_ro/cfs/cdirs/desi/users/rongpu/data/dr11dev/sky_templates_raw'
plot_dir = '/global/cfs/cdirs/cosmo/www/temp/rongpu/dr11/sky_templates_raw'
...
def make_plots(run):
    ...
    sky_path = os.path.join(template_dir, 'sky_raw_{}_{}.fits'.format(band, run))

    plot_path = os.path.join(plot_dir, band, '{}_{}_sky_template_raw.png'.format(band, run))
    ...
    for ii, ccdnum in enumerate(ccdnum_list):
        ...
        try:
            ...
            header = fitsio.read_header(sky_path, ext=ccdname)
        except:
        ...
        mjd_min = np.minimum(header['MJDMIN'], mjd_min)
        mjd_max = np.maximum(header['MJDMAX'], mjd_max)
    ...
    mjd_span = mjd_max - mjd_min
    text += 'observed over {:.1f} days\n'.format(mjd_span)
    plt.text(1.04, 0.810, text, fontsize=34)
...
def main():
    ...
    for run in np.unique(skyrun['run']):
        mask = skyrun['run']==run
        band = skyrun['filter'][mask][0]
        template_path = os.path.join(template_dir, 'sky_raw_{}_{}.fits'.format(band, run))
