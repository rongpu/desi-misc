...
plot_dir = '/global/cfs/cdirs/cosmo/www/temp/rongpu/dr11/sky_templates'
...
def main():
    ...
    with Pool(processes=n_processes) as pool:
        res = pool.map(make_plots, np.unique(skyrun['run']), chunksize=1)

    # for run in np.unique(skyrun['run']):
    #     mask = skyrun['run']==run
    #     band = skyrun['filter'][mask][0]
    #     template_path = os.path.join(template_dir, 'sky_template_{}_{}.fits'.format(band, run))
