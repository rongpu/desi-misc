...
blob_dir_dr9 = '/dvs_ro/cfs/cdirs/desi/users/rongpu/dr9/decam_ccd_blob_mask'
blob_dir_dr11 = '/dvs_ro/cfs/cdirs/desi/users/rongpu/data/dr11dev/decam_ccd_blob_mask'

image_dir = '/dvs_ro/cfs/cdirs/cosmo/staging'
##################################################################################################
image_dir_wihtout_cp_fringe = '/global/cfs/cdirs/desicollab/users/rongpu/data/dr11dev/undo_cp_fringe_corr'
##################################################################################################
...
output_dir = '/global/cfs/cdirs/desi/users/rongpu/data/dr11dev/sky_templates_raw'

# ccd_columns = ['image_filename', 'image_hdu', 'expnum', 'ccdname', 'filter', 'ccd_cuts', 'ccdskycounts', 'plver']
ccd_columns = ['image_hdu', 'expnum', 'ccdname', 'ccdskycounts', 'ra', 'dec', 'exptime', 'ccd_cuts']
ccd = Table(fitsio.read(surveyccd_path, columns=ccd_columns))
print(len(ccd))

# Apply ccd_cuts but ignore DEPTH_CUT
ccd_cuts_bits = np.arange(20)
bits_to_ignore = [14]
ccd_cuts_bits = ccd_cuts_bits[~np.in1d(ccd_cuts_bits, bits_to_ignore)]
print(ccd_cuts_bits)

mask_clean = np.ones(len(ccd), dtype=bool)
for bit in ccd_cuts_bits:
    mask_clean &= (ccd['ccd_cuts'] & 2**bit)==0
print(np.sum(~mask_clean)/len(mask_clean))
ccd = ccd[mask_clean]

# skyrun = Table.read('/global/cfs/cdirs/desicollab/users/schlafly/decals/skyrunsdr11-v3.fits')
skyrun = Table.read('/global/cfs/cdirs/desicollab/users/rongpu/data/dr11dev/skyrunsdr11-v3-rearrange.fits')
print(len(skyrun))
...
# Add ccd_cuts to skyruns
skyrun_expnum = np.array(skyrun['expnum'])
_, idx = np.unique(ccd['expnum'], return_index=True)
exp = ccd[idx].copy()
skyrun = join(skyrun, exp[['expnum', 'ccd_cuts']], keys='expnum', join_type='left')
assert np.all(skyrun_expnum==skyrun['expnum'])  # sanity check
...
def get_median_stacked_image(ccdnum):
    ...
    if ccdnum<=32:
        time.sleep(1.5*ccdnum_list.index(ccdnum))
    ...
    for index, skyrun_index in enumerate(range(len(skyrun))):
        ...
        header_keys = fitsio.read_header(img_fn, ext=1).keys()
        if ('FRGSCALE' in header_keys) and ('FRGSCNEW' not in header_keys):
            img_fn = os.path.join(image_dir_wihtout_cp_fringe, skyrun['image_filename'][skyrun_index]).strip()
        ...
        if ccdname=='S7':  # Make the bad half of the S7 NaN
            half = img.shape[1] // 2
            img[:, half:] = np.nan
        ...
        blob_path_dr11 = os.path.join(blob_dir_dr11, 'blob_mask', img_filename_base+'-blobmask.npz')
        if os.path.isfile(blob_path_dr9) and (os.stat(blob_path_dr9).st_size!=0):
            blob_path = blob_path_dr9
        elif os.path.isfile(blob_path_dr11) and (os.stat(blob_path_dr11).st_size!=0):
            blob_path = blob_path_dr11
        else:
            print(img_filename_base+'-blobmask.npz (expnum={}) does not exist!'.format(expnum))
            continue
        ...
        ccdskycounts_median = np.nanmedian(ccd1['ccdskycounts'])
        # print('ccdskycounts_median = {:.4f}'.format(ccdskycounts_median))

        # Normalize by ccdskycounts and exptime
        img = img/ccdskycounts_median/ccd1['exptime'][0]*100.

        # Apply blob and ood mask
        img_mask = (blob==True) & (ood==0)
        # Remove median sky
        sky = np.nanmedian(img[img_mask].flatten())
        img = img - sky
        img[~img_mask] = np.nan
    ...
    header['MJDMIN'] = skyrun['mjd_obs'].min()
    header['MJDMAX'] = skyrun['mjd_obs'].max()
...
if len(skyrun)>max_n_exposure:
    ...
    # prioritize ccd_cuts==0 exposures
    idx = np.where(skyrun['ccd_cuts']==0)[0]
    if len(idx)<max_n_exposure:
        idx1 = np.where(skyrun['ccd_cuts']>0)[0]
        idx1 = np.random.choice(idx1, size=max_n_exposure-len(idx), replace=False)
        idx = np.concatenate([idx, idx1])
    elif len(idx)>max_n_exposure:
        idx = np.random.choice(idx, size=max_n_exposure, replace=False)
    assert len(idx)==max_n_exposure
    idx.sort()
    skyrun = skyrun[idx]

print('{} exposures'.format(len(skyrun)))
...
with Pool(processes=n_processes) as pool:
    res = pool.map(get_median_stacked_image, ccdnum_list, chunksize=1)
