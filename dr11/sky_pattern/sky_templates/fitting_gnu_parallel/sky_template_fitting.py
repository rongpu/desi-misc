...
skyscale_dir = '/global/cfs/cdirs/desi/users/rongpu/data/dr11dev/sky_scales_tmp/'
...
if not os.path.isfile(sky_path):
    raise ValueError(sky_path, 'not found')
...
def template_fitting(expnum, diagnostic_touch=True):
    ...
    if ('FRGSCALE' in header_keys) and ('FRGSCNEW' not in header_keys):
        ...
        if not os.path.isfile(image_path):
            raise ValueError('Image file does not exist: ', image_path)
    ##########################################################################################################
    ...
    for ii, ccdnum in enumerate(ccdnum_list):
        ...
        if len(tmp)==0:
            print('{} in expnum {} does not exist'.format(ccdname, expnum), image_path)
            continue
        ccd_index = tmp[0]
        ...
        if np.sum(~img_mask)/np.prod(img_mask.shape)>0.9:
            print('{} {} {:.3f}% pixels are masked; skip'.format(expnum, ccdname, np.sum(~img_mask)/np.prod(img_mask.shape)*100))
            continue
