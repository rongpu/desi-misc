...
metrics_dir = '/dvs_ro/cfs/cdirs/cosmo/work/legacysurvey/dr11-early/metrics'
coadd_dir = '/dvs_ro/cfs/cdirs/cosmo/work/legacysurvey/dr11-early/coadd'

# surveyccd_path = '/dvs_ro/cfs/cdirs/cosmo/work/legacysurvey/dr11-early/survey-ccds-decam-dr11-merged-incl-early.fits'
output_dir = '/global/cfs/cdirs/desi/users/rongpu/data/dr11dev/decam_ccd_blob_mask'
touch_dir = '/pscratch/sd/r/rongpu/tmp/blobmask_being_written'

output_dir_dr9 = '/dvs_ro/cfs/cdirs/desi/users/rongpu/dr9/decam_ccd_blob_mask'
...
surveyccd_path = '/global/cfs/cdirs/desi/users/rongpu/data/dr11dev/sky_pattern/survey_ccds_chunks/survey-ccds-decam-dr11-merged-incl-early-{}.fits'.format(task_id)
...
def save_ccd_blob_mask(expnum):
    ...
    for loop_index, ccd_index in enumerate(ccd_idx):
        ...
        for brick_index in brick_idx:
            ...
            maskbits_path = os.path.join(coadd_dir, '{}/{}/legacysurvey-{}-maskbits.fits.fz'.format(brickname[:3], brickname, brickname))

            if os.path.isfile(blobs_path):
                blobs = fitsio.read(blobs_path)
                maskbits = fitsio.read(maskbits_path)
                blob_hdu = fits.open(blobs_path)
                mask_bad = (blobs==1)  # blobmask in early coadds is binary 0/1
    ...
    if not all_ccds_empty:
        if diagnostic_touch:
            ...
            Path(os.path.join(touch_dir, os.path.basename(output_path))).touch()
        np.savez_compressed(output_path, **data)
        if diagnostic_touch:
            os.remove(os.path.join(touch_dir, os.path.basename(output_path)))
