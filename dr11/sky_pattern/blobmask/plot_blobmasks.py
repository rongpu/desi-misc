...
plot_dir = '/global/cfs/cdirs/desicollab/users/rongpu/data/dr11dev/decam_ccd_blob_mask/plots/focal_plane'
...
def blobmask_plot(image_filename, figsize=(26, 24), binsize=10, median=True,
    show=False):
    '''
...
mask = ccd['ccd_cuts']==0
tmp = Table()
tmp['expnum'], tmp['count'] = np.unique(ccd['expnum'][mask], return_counts=True)
...
np.random.seed(888)
expnum_list = np.random.choice(expnum_list, size=len(expnum_list), replace=False)  # shuffle

np.random.seed(213)
idx = np.random.choice(len(expnum_list), size=200, replace=False)
...
# shuffle
idx = np.where(len(ccd), size=len(ccd), replace=False)
ccd = ccd[idx]
print(len(ccd))

img_fns = np.unique(ccd['image_filename'])

n_processes = 128
with Pool(processes=n_processes) as pool:
    res = pool.map(blobmask_plot, img_fns)
