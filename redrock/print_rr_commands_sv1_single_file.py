# A single coadd per exposure instead of one file for each petal
########## MAY CAUSE OUT-OF-MEMORY WITH DEEP COADDS!!! #########
...
for tileid in np.unique(cframes['tileid']):
    ...
    # for petal_loc in range(10):
    petal_loc = 0  # assuming petal 0 exists; petal_loc is not really used
    ...
    for subset_index in range(len(subset_split)):
        ...
        for index in range(len(subset)):
            ...
            input_argument += os.path.join(exposure_dir.replace(redux_dir, '$REDUXDIR'), 'cframe-*.fits ')
        ...
        if n_exp==np.inf:
            if len(obsdate_list)==1:
                ...
                output_argument = os.path.join('$OUTDIR', str(tileid), 'coadd-{}-{}.fits'.format(tileid, obsdate_list[0]))
            else:
                output_argument = os.path.join('$OUTDIR', str(tileid), 'coadd-{}.fits'.format(tileid))
        elif n_exp==1:
            exposure = os.path.basename(exposure_dir)
            output_argument = os.path.join('$OUTDIR', str(tileid), 'coadd-{}.fits'.format(exposure))
        else:
            output_argument = os.path.join('$OUTDIR', str(tileid), 'coadd-{}exp-subset-{}.fits'.
            format(n_exp, subset_index))
