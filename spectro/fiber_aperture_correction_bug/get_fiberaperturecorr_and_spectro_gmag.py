...
filter_curves = dict()
for band in ['g', 'r', 'i', 'z']:
    filter_name = 'decamDR1-'+band
    filter_curves[band] = speclite.filters.load_filter(filter_name)
...
def get_data(index):
    ...
    for fn in fns:
        ...
        for index in range(len(flux)):
            gmag.append(compute_ab_mag(wave, flux[index], filt_ww, filt_tt))
        tmp2['desi_gmag'] = np.array(gmag)
...
np.random.seed(888)
idx = np.random.choice(len(tiles), size=128, replace=False)
n_process = 128
with Pool(processes=n_process) as pool:
    res = pool.map(get_data, idx)
cat = vstack(res, join_type='exact')

cat.write('/pscratch/sd/r/rongpu/tmp/fiberapcorr_and_spectro_gmag.fits')
