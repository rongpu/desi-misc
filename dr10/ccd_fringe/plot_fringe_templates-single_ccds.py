...
fringe_templates_dr9_dir = '/global/cfs/cdirs/desi/users/rongpu/dr9/fringe/DECam_CP-Fringe-Normed'

plot_dir = '/global/cfs/cdirs/desi/users/rongpu/dr10dev/fringe/plots'


# for ccdnum in range(1, 63):
def plot_fringe(ccdnum):

    ccdname = ccdnamenumdict_inv[ccdnum]
    print(ccdnum, ccdname)

    img_path = os.path.join(fringe_templates_raw_dir, 'DECam_z_frg_{}_CCD{}_raw.fits'.format(ccdname, str(ccdnum).zfill(2)))
    if os.path.isfile(img_path):
        img = fitsio.read(img_path)
        create_image((img).T, cmap='seismic', vmin=-vrange_raw, vmax=vrange_raw)
        plt.savefig(os.path.join(plot_dir, 'DECam_z_frg_{}_CCD{}_raw.png'.format(ccdname, str(ccdnum).zfill(2))))
        plt.close()

    img_path = os.path.join(fringe_templates_dir, 'DECam_z_frg_{}_CCD{}.fits'.format(ccdname, str(ccdnum).zfill(2)))
    if os.path.isfile(img_path):
        img = fitsio.read(img_path)
        create_image((img).T, cmap='seismic', vmin=-vrange_smooth, vmax=vrange_smooth)
        plt.savefig(os.path.join(plot_dir, 'DECam_z_frg_{}_CCD{}.png'.format(ccdname, str(ccdnum).zfill(2))))
        plt.close()

    img_path = os.path.join(fringe_templates_dr9_dir, 'DECam_z_frg_{}_CCD{}.fits'.format(ccdname, str(ccdnum).zfill(2)))
    if os.path.isfile(img_path):
        img = fitsio.read(img_path)
        create_image((img).T, cmap='seismic', vmin=-vrange_smooth, vmax=vrange_smooth)
...
with Pool(processes=n_processes) as pool:
    res = pool.map(plot_fringe, np.arange(1, 63), chunksize=1)
