# No ood mask for stacking (but use it for smooth sky)
...
surveyccd_path = '/global/cfs/cdirs/desi/users/rongpu/data/dr10dev/deep_fields/misc/survey-ccds-dr10-deep-fields-v1-z-fringe-stacking-2.fits'
...
output_path = os.path.join(output_dir, 'DECam_z_frg_{}_CCD{}_raw_no_norm-v5.fits'.format(ccdname, str(ccdnum).zfill(2)))
...
plt.savefig(os.path.join(plot_dir, 'fringe_new_{}_no_norm-v5.png'.format(ccdnum)))
plt.close()
...
plt.savefig(os.path.join(plot_dir, 'fringe_new_{}_smooth_no_norm-v5.png'.format(ccdnum)))
plt.close()
