...
output_dir = '/global/cfs/cdirs/desi/users/rongpu/dr10dev/fringe/data/frgscale_init_test1'
...
for ccdnum in range(1, 63):
    ...
    if os.path.isfile(fringe_template_path):
        ...
        trim_size_x = 0
        trim_size_y = 0
        fringe_spline_data = np.nanmedian(np.nanmedian(fringe_tmp.reshape((fringe_tmp.shape[0]//binsize, binsize, fringe_tmp.shape[1]//binsize,-1)), axis=3), axis=1)
        ...
        fringe_spline = spline(np.arange(fringe_tmp.shape[0]), np.arange(fringe_tmp.shape[1]))
        # Subtract spline
        fringe_tmp -= fringe_spline
