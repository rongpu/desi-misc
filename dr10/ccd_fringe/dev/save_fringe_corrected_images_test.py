...
image_output_dir = '/pscratch/sd/r/rongpu/dr10dev/fringe_corrected_images_test'
...
def save_image(expnum):
    ...
    for hdu_index in range(len(hdul_r)):
        if hdu_index==0:
            ...
            if np.sum(mask)==1:
                frgscale = np.clip(fringe_table['frgscale'][mask][0], 0, 2*fringe_table['frgscale_apply'][mask][0])
            else:
