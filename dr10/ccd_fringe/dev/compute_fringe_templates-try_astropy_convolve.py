...
surveyccd_path = '/global/cfs/cdirs/desi/users/rongpu/data/dr10dev/deep_fields/misc/survey-ccds-dr10-deep-fields-v1-z-fringe-stacking-1.fits'
...
output_path = os.path.join(output_dir, 'DECam_z_frg_{}_CCD{}_raw_no_norm_test10.fits'.format(ccdname, str(ccdnum).zfill(2)))
...
def process_ccd(ccd_index):
    ...
    # Normalize by exptime*ccdskycounts
    img = img/(ccd['exptime'][ccd_index]*ccd['ccdskycounts'][ccd_index])
    ...
    img1_smooth = convolve(img1, kernel, boundary='extend', nan_treatment='interpolate')
...
plt.savefig(os.path.join(plot_dir, 'fringe_new_{}_no_norm_test10.png'.format(ccdnum)))
plt.close()
...
plt.savefig(os.path.join(plot_dir, 'fringe_new_{}_smooth_no_norm_test10.png'.format(ccdnum)))
plt.close()
