...
output_path = os.path.join(output_dir, 'DECam_z_frg_{}_CCD{}_raw_no_ood.fits'.format(ccdname, str(ccdnum).zfill(2)))
...
def process_ccd(ccd_index):
    ...
    # Remove median sky
    sky = np.median(img[mask_sky].flatten())
    img = img - sky
...
plt.savefig(os.path.join(plot_dir, 'fringe_new_{}_no_ood.png'.format(ccdnum)))
plt.close()
...
plt.savefig(os.path.join(plot_dir, 'fringe_new_{}_smooth_no_ood.png'.format(ccdnum)))
plt.close()
