...
export LEGACY_SURVEY_DIR=$SCR/tractor/deep_fields/cosmos/test/test-1-test
outdir=$LEGACY_SURVEY_DIR

export CACHE_DIR=$SCR/tractor/deep_fields/cosmos/cache_delete
...
python -O $LEGACYPIPE_DIR/legacypipe/runbrick.py \
     ...
     --max-memory-gb 100 \
     --threads "${ncores}" \
     --blob-mask-dir /global/u2/r/rongpu/misc/dr9-blobmask-south \
      >> "$log" 2>&1
