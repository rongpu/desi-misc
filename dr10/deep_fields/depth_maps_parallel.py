...
for band in ['g', 'r', 'i', 'z', 'Y']:
    for field_index in range(len(field_names)):
        tasks.append([band, field_index])

print('Start!')
time_start = time.time()
n_processes = 128
with Pool(processes=n_processes) as pool:
    res = pool.starmap(get_depth_map, tasks)
res_dict = {}
for index, task in enumerate(tasks):
    band, field_index = task
    res_dict[band+str(field_index)] = res[index]
...
for band in ['g', 'r', 'i', 'z', 'Y']:
    ...
    for field_index in range(len(field_names)):
        ...
        nexp_grid, efftime_grid, depth_grid, seeing_grid, extent = res_dict[band+str(field_index)]
