# Upweight shallower bricks to avoid chip gaps in the final footprint
...
def get_brickscore(ccd_index):
    ...
    if np.all(bricks['nccd'][mask]<nccd_threshold):
        return -np.inf
    brickscore = np.sum(bricks['nccd'][mask]-nccd_threshold) - 100*np.sum(bricks['depth'][mask]<1.0)
...
with Pool(processes=n_processes) as pool:
    res = np.array(pool.map(get_nccd, ccds_to_keep)).T
bricks['nccd'] = np.sum(res, axis=1)
bricks['efftime'] = np.dot(res, ccd['efftime'][ccds_to_keep])
...
ccd.write('/global/cfs/cdirs/desi/users/rongpu/dr10dev/deep_fields/tmp/survey-ccds-{}-{}-new-2.fits'.format(field_index, band), overwrite=True)
