...
surveyccd_path = '/global/cfs/cdirs/desi/users/rongpu/dr10dev/sky_pattern/survey_ccds_chunks_z/survey-ccds-decam-dr10-v2-z-{}.fits'.format(task_id)
...
def save_ccd_blob_mask(expnum):
    ...
    if (not overwrite) and os.path.isfile(output_path) and os.stat(output_path).st_size!=0:
        time_modified = os.path.getmtime(output_path)
        if time_modified>1644107000:  # Don't overwrite new files
            return None
        blob_data = np.load(output_path)
        if len(blob_data)>=50:
            n_good_ccd = 0
            for key in list(blob_data.keys()):
                blob = blob_data[key]
                if np.sum(~blob)/np.prod(blob.shape)<0.9:
                    n_good_ccd += 1
            if n_good_ccd>=50:
                return None
