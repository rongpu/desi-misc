...
def get_systematics(pix_idx):
    ...
    for index in np.arange(len(pix_idx)):
        ...
        for band in ['g', 'r', 'z']:
            hp_table[band+'mag_diff_median'][index] = np.nanmedian(dr10[band+'mag_diff'][idx])
            hp_table[band+'mag_diff_mean'][index] = np.nanmean(dr10[band+'mag_diff'][idx])
            hp_table[band+'mag_diff_std'][index] = np.nanstd(dr10[band+'mag_diff'][idx])
            hp_table[band+'mag_diff_nmad'][index] = nmad(dr10[band+'mag_diff'][idx])
            hp_table[band+'mag_n_objects'][index] = np.sum(np.isfinite(dr10[band+'mag_diff'][idx]))
...
for band in ['g', 'r', 'z']:
    ...
    dr10[band+'mag_diff'][mask] = np.nan
...
for nside in [64, 128, 256]:
    ...
    pix_allobj = hp.pixelfunc.ang2pix(nside, dr10['RA'], dr10['DEC'], lonlat=True)
    ...
    hp_table.write('/global/cfs/cdirs/desi/users/rongpu/data/gaia_dr3/misc/more/gaia_xp_dr10_offset_maps_{}-common_stars.fits'.format(nside), overwrite=True)
