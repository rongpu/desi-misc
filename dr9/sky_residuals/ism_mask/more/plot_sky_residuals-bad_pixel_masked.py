...
plot_dir = '/global/cfs/cdirs/desi/users/rongpu/imaging_sys/tmp/sky_residuals'
...
for nside in [512]:
    ...
    mask = ~np.in1d(sky['HPXPIXEL'], bad_pixels)
    sky = sky[mask]
    ...
    for band in ['g', 'r', 'z']:
        ...
        plot_path = os.path.join(plot_dir, 'dr9_sky_median_{}_{}_{}.png'.format(band, nside, mask_str))
        if not os.path.isfile(plot_path):
            plot_map(nside, sky['sky_median_'+band], pix=sky['HPXPIXEL'],
                     ...
                     title='sky_median_{} NSIDE={}'.format(band, nside), save_path=plot_path, xsize=6000)

        plot_path = os.path.join(plot_dir, 'dr9_sky_nmad_{}_{}_{}.png'.format(band, nside, mask_str))
        if not os.path.isfile(plot_path):
            plot_map(nside, sky['sky_nmad_'+band], pix=sky['HPXPIXEL'],
                     ...
                     title='sky_nmad_{} NSIDE={}'.format(band, nside), save_path=plot_path, xsize=6000)
