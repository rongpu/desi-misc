...
def decam_stitch_and_plot(exposure, binsize=120, plot_path=None, save_path=None, vrange=None, dr8=False, median=True,
    blob_mask=True, ood_mask=True, show=False):
    '''
    Example:
    decam_stitch_and_plot(781475, plot_path='tmp_stitch.png', save_path='stitch.npz')
    '''
    if type(exposure)==str:
        image_path = os.path.join(image_dir, exposure.strip())
    elif isinstance(exposure, int) or isinstance(exposure, np.integer):
    ...
    fullimg_original = fullimg.copy()
    fullimg[~np.isfinite(fullimg)] = 0

    if save_path is not None:
        np.savez_compressed(save_path, data=fullimg_original)
